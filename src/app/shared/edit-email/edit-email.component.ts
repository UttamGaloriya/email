...
export class EditEmailComponent {
  ...
  ngOnInit() {
    this.emailFormControl = new FormControl(this.emailName, [Validators.required, Validators.pattern('^[a-z0-9._%+-]+@[a-z0-9.-]+\\.[a-z]{2,4}$')]);
  }

  submitForm(value: string) { this.submit.emit(value) }
  ...
  get emailChange() {
    if (!this.emailFormControl.valid) {
      ...
      if (this.emailName === this.emailFormControl.value) {
        return true
      }
      return false
    }
